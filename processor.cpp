...
const static uint64_t KERNELPAGES = 1; //1024byte pages need 1 page
const static uint64_t STACKPAGES = 1;
const static uint64_t FREEPAGES = 12;
const static uint64_t BASEPAGES = 3;

using namespace std;

Processor::Processor(Tile *parent, MainWindow *mW, uint64_t numb):
    masterTile(parent), mode(REAL), router(parent->getColumn(),
    parent->getRow()), mainWindow(mW), processorNumber(numb)
{
...
void Processor::pcAdvance(const long count)
{
	...
	fetchAddressRead(programCounter, true, false);
}

void Processor::setProgramCounter(const uint64_t& address)
{
       programCounter = address;
       fetchAddressRead(address, true, false);
}
